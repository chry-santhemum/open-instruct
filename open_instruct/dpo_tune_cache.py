...
def main(args: dpo_utils.ExperimentConfig, tc: TokenizerConfig):
    ...
    if args.loss_type.needs_reference_model:
        if args.reference_logprobs_cache_path is not None:
            ...
            old_cache = model_utils.TensorCache.from_disk(
                args.reference_logprobs_cache_path, device=accelerator.device
            )
            old_cache.offset = old_cache.tensors["chosen_logps"].shape[0]
            reference_cache = dpo_utils.append_reference_logprobs_cache(
                model=model,
                ...
                original_logprobs_cache=old_cache,
