...
def append_reference_logprobs_cache(
    model: torch.nn.Module,
    dataloader: torch.utils.data.DataLoader,
    accelerator: Accelerator,
    average_log_prob: bool,
    ...
    original_logprobs_cache: model_utils.TensorCache,
    use_lora: bool = False,
) -> model_utils.TensorCache:
    """Append the reference logprobs cache with new samples."""
    if new_dataset_size == 0:
        return original_logprobs_cache

    model.eval()
    device = accelerator.device
    chosen_tensor = torch.full((new_dataset_size,), float("-inf"), dtype=torch.float32, device=device)
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            if use_lora:
                with accelerator.unwrap_model(model).disable_adapter():
                    chosen_logps, rejected_logps, _ = forward_fn(model, batch_new, average_log_prob=average_log_prob)
            else:
                chosen_logps, rejected_logps, _ = forward_fn(model, batch_new, average_log_prob=average_log_prob)

            chosen_tensor[batch_new["new_index"]] = chosen_logps
            rejected_tensor[batch_new["new_index"]] = rejected_logps
